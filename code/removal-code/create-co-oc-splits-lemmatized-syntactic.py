...
def contains_co_occurrence(lemmas, sentence):
    ...
    if len(concepts_seen) > 0:
        ...
        for concept, feature in pairs_:
            if feature in lemmas:
                return syntactically_related(sentence, concept, feature)
    return False

def syntactically_related(sentence, concept, feature):
    tree = lemmatizer(sentence).sentences[0]
    concept_heads = []
    feature_heads = []
    concept_ids = []
    feature_ids = []
    for word in tree.words:
        if word.lemma is None:
            continue
        if word.lemma.lower() == concept:
            concept_heads.append(word.head)
            concept_ids.append(word.id)
        if word.lemma.lower() == feature:
            feature_heads.append(word.head)
            feature_ids.append(word.id)
    if len(set(concept_ids).intersection(set(feature_heads))) > 0:
        return True
    if len(set(concept_heads).intersection(set(feature_ids))) > 0:
        return True
    return False
...
def get_replacements(reserve_sentences, k):
    ...
    for instance in reserve_sentences:
        ...
        if contains_co_occurrence(lemmas, sentence):
            continue
        else:
